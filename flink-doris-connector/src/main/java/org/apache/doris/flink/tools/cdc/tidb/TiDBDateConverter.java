...
public class TiDBDateConverter implements CustomConverter<SchemaBuilder, RelationalColumn> {
    private static final Logger log = LoggerFactory.getLogger(TiDBDateConverter.class);
    ...
    static {
        ...
        DEFAULT_PROPS.setProperty("date.type", "org.apache.doris.flink.tools.cdc.tidb.TiDBDateConverter");
    ...
    public void converterFor(RelationalColumn column, ConverterRegistration<SchemaBuilder> registration) {
        String tidbSqlType = column.typeName().toUpperCase();
        SchemaBuilder schemaBuilder = null;
        Converter converter = null;
        if ("DATE".equals(tidbSqlType)) {
            schemaBuilder = SchemaBuilder.string().optional();
            converter = this::convertDate;
        }
        if ("TIME".equals(tidbSqlType)) {
            schemaBuilder = SchemaBuilder.string().optional();
            converter = this::convertTime;
        }
        if ("DATETIME".equals(tidbSqlType)) {
            schemaBuilder = SchemaBuilder.string().optional();
            converter = this::convertDateTime;
        }
        if ("TIMESTAMP".equals(tidbSqlType)) {
            schemaBuilder = SchemaBuilder.string().optional();
